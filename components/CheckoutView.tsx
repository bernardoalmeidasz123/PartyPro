...
const CheckoutView: React.FC<CheckoutViewProps> = ({ onBack }) => {
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    const newActivationEvent: EventParty = {
      ...
      totalBudget: 99.90,
      totalSupplierCost: 0,
      notes: `Aguardando ativação manual do Admin Bernardo. Comprovante registrado.`,
  ...
  if (step === 'processing') {
    return (
      <div className="min-h-screen bg-emerald-950 flex flex-col items-center justify-center p-8 text-center">
        ...
        <h2 className="text-2xl font-display text-white">Validando Comprovante...</h2>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-8">
        <div className="max-w-2xl w-full text-center space-y-8 animate-in fade-in zoom-in duration-700">
          ...
          <h2 className="text-4xl font-display text-emerald-950">Solicitação Enviada!</h2>
          
          <div className="bg-slate-50 p-10 rounded-[40px] border border-slate-100 text-left space-y-6 shadow-sm">
            <p className="text-slate-500 text-sm">
              Um novo item de <strong>Ativação Pendente</strong> foi inserido na programação do sistema. 
              O Administrador Bernardo irá aprovar o acesso manualmente após a conferência do pagamento.
            </p>
            
            <div className="p-5 bg-emerald-50 rounded-2xl border border-emerald-100">
              <p className="text-[10px] font-black text-emerald-600 uppercase mb-2">Link de Cadastro (após aprovação):</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[40px] shadow-2xl overflow-hidden p-10">
        <h3 className="text-2xl font-bold text-slate-800 font-display mb-6">Simulador de Pagamento</h3>
        <form onSubmit={handlePayment} className="space-y-6">
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase">E-mail da Compra</label>
            <input 
              type="email" 
              required 
          ...
          <button type="submit" className="w-full bg-emerald-950 text-white py-4 rounded-2xl font-bold shadow-xl">
            Confirmar Envio p/ Bernardo
          </button>
          <button type="button" onClick={onBack} className="w-full text-slate-400 text-sm font-bold">Voltar</button>
        </form>
      </div>
    </div>
  );
