  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (authorized.includes(inputEmail)) {
        ...
        setError('Acesso pendente de aprovação manual pelo Administrador Bernardo.');
      }
      setIsChecking(false);
    }, 600);
  };
      ...
      <div className="md:w-1/2 bg-emerald-950 p-12 text-white flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="z-10 space-y-6">
          ...
          <p className="text-emerald-300 max-w-md text-lg opacity-70 leading-relaxed">
            Plataforma dedicada a decoradores de alto padrão. Agenda Pro 2026-2030 ativa.
          </p>
        </div>

        <div className="z-10 flex items-center gap-3">
           <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_10px_#10b981]"></span>
           <p className="text-[10px] uppercase tracking-widest text-emerald-600 font-bold">Ambiente Seguro • Bernardo Almeida</p>
        </div>
      </div>
      ...
      <div className="md:w-1/2 flex items-center justify-center p-8 lg:p-16">
        <div className="w-full max-w-sm space-y-12">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-400 text-sm mt-3">Entre com suas credenciais ou solicite acesso.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isChecking}
              className="w-full py-4 rounded-2xl font-bold bg-emerald-950 text-white shadow-xl hover:bg-emerald-900 transition-all"
            >
          ...
          >
            Acesso Fornecedor Convidado
          </button>

          <div className="text-center">
            <p className="text-slate-400 text-xs">
              Não tem acesso? <button onClick={onGoToRequest} className="text-emerald-700 font-bold hover:underline">Solicitar Aprovação Manual</button>
            </p>
          </div>
        </div>
      </div>
